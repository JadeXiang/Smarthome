...
function initModel(model_address, sc, x, y, z, rx, ry, rz) {
    ...
    loader.load(model_address,function (loadedMesh) {
        ...
        */
        ...
        modelMesh.name = modelNum;

        box[modelNum] = new THREE.Box3().setFromObject(modelMesh);
        modelNum++;
...
function initDragControls() {
    ...
    dragControls.addEventListener('dragstart', function (event) {
        if(isChecked == true){
            ...
            changeMaterial(event.object);
            intersects.push(event.object);
            originPosition = event.object.position;
    ...
    dragControls.addEventListener('dragend', function (event) {
        ...
        // for (var i = 0; i < intersects.length; i++) 
        //     {
        //         if (event.object.position == intersects[i].position) 
        //         {
        //             intersects.pop(intersects[i]);
        //         }
        //     } 
...
function boxMove(event){
    ...
    var i = event.object.name;
    box[i] = new THREE.Box3().setFromObject(event.object);
    var flag;
    for (var n=0; n<box.length; n++){
      flag = box[i].intersectsBox(box[n]);
      //console.log(flag)
      if(i != n && flag){
        event.object.position.set(ox, oy, oz);
        break;
      }
    }
}
